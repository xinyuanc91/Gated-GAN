...
local opt_train = {
   ...
   save_display_feq = 2500,    -- save the current display of results every save_display_freq_iterations
   continue_train = 0,          -- if continue training, load the latest model: 1: true, 0: false
   serial_batches = 0,          -- if 1, takes images in order to make batches, otherwise takes them randomly
   checkpoints_dir = './checkpoints', -- models are saved here
   cudnn = 1,                         -- set to 0 to not use cudnn
   which_model_netD = 'basic',        -- selects model to use for netD
   which_model_netG = 'auto_gated_resnet_6blocks',   -- selects model to use for netG
   norm = 'instance',             -- batch or instance normalization
   n_layers_D = 3,                -- only used if which_model_netD=='n_layers'
   lambda_A = 10.0,               -- for cycleGAN: weight for cycle loss (A -> B -> A); for gated-gan:weight for auxilary classifier
   lambda_B = 10.0,               -- no use for gated-gan
   model = 'gated_gan',           -- which mode to run. 'gated-gan'
   ...
   resize_or_crop = 'resize_and_crop',  -- resizing/cropping strategy: scale_width
   autoencoder_constrain = 10 ,

   n_style = 4,                   -- multi-style number
   start_epoch=1,
   valid_freq =0,
   test_data_path = '', --'',
   G_ac=1,
   L2_loss=0,  -- the loss of autoencoder reconstruction loss. 0=L1 loss, 1=L2 loss.
   tv_strength=1e-6,
   save_param=0,
}
...
local opt_test = {
  ...
  model = 'gated_gan',               -- which mode to run. training for 'gated_gan'; to use test gategan model, select `gatedgan_test`
  align_data = 0,                    -- if > 0, use the dataloader for pix2pix
  multi_data=1,                     -- if >0, use the datqloader for gated-gan
  which_direction = 'AtoB',          -- AtoB or BtoA
  resize_or_crop = 'resize_and_crop',  -- resizing/cropping strategy
  n_style = 4,
  which_model_netG = 'gated_resnet_6blocks',
}
