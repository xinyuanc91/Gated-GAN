...
function defineG(input_nc, input_nclass, output_nc, ngf, which_model_netG, nz, arch)
  local netG = nil
  print(which_model_netG)
  if which_model_netG == "gated_resnet_6blocks" then netG = defineG_gated_resnet_6blocks(input_nc, input_nclass, output_nc, ngf);netG:apply(weights_init)
  elseif which_model_netG == "auto_gated_resnet_6blocks" then netG=defineG_auto_gated_resnet_6blocks(input_nc, input_nclass, output_nc, ngf);netG:apply(weights_init)
  else error("unsupported netG model")
  end
  return netG
end

function defineD(input_nc, ndf, which_model_netD, n_layers_D, use_sigmoid, add_class, input_nclass)
  local netD = nil
  ...
  if add_class then 
    netD = concat_class_layer(netD, use_sigmoid, n_layers_D, ndf, input_nclass)
  end
  netD:apply(weights_init)
  return netD
end

function defineG_encoder_decoder(input_nc, input_nclass, output_nc, ngf)
    ...
    local s1 = - nn.SpatialConvolution(input_nclass, ngf, 4, 4, 2, 2, 1, 1)
    -- input is (ngf) x 128 x 128
    local s2 = s1 - nn.LeakyReLU(0.2, true) - nn.SpatialConvolution(ngf, ngf * 2, 4, 4, 2, 2, 1, 1) - normalization(ngf * 2)
    -- input is (ngf * 2) x 64 x 64
    local s3 = s2 - nn.LeakyReLU(0.2, true) - nn.SpatialConvolution(ngf * 2, ngf * 4, 4, 4, 2, 2, 1, 1) - normalization(ngf * 4)
    -- input is (ngf * 4) x 32 x 32
    local s4 = s3 - nn.LeakyReLU(0.2, true) - nn.SpatialConvolution(ngf * 4, ngf * 8, 4, 4, 2, 2, 1, 1) - normalization(ngf * 8)
    -- input is (ngf * 8) x 16 x 16
    local s5 = s4 - nn.LeakyReLU(0.2, true) - nn.SpatialConvolution(ngf * 8, ngf * 8, 4, 4, 2, 2, 1, 1) - normalization(ngf * 8)
    -- input is (ngf * 8) x 8 x 8
    local s6 = s5 - nn.LeakyReLU(0.2, true) - nn.SpatialConvolution(ngf * 8, ngf * 8, 4, 4, 2, 2, 1, 1) - normalization(ngf * 8)
    -- input is (ngf * 8) x 4 x 4
    local s7 = s6 - nn.LeakyReLU(0.2, true) - nn.SpatialConvolution(ngf * 8, ngf * 8, 4, 4, 2, 2, 1, 1) - normalization(ngf * 8)
    -- input is (ngf * 8) x 2 x 2
    local s8 = s7 - nn.LeakyReLU(0.2, true) - nn.SpatialConvolution(ngf * 8, ngf * 8, 4, 4, 2, 2, 1, 1) -- normalization(ngf * 8)
    -- input is (ngf * 8) x 1 x 1

    local t = {e8,s8} - nn.JoinTable(2)



    local d1 = t - nn.ReLU(true) - nn.SpatialFullConvolution(ngf * 8 * 2, ngf * 8 , 4, 4, 2, 2, 1, 1) - normalization(ngf * 8) - nn.Dropout(0.5)
    -- input is (ngf * 8) x 2 x 2
    ...
    local netG = nn.gModule({e1,s1},{o1})
    return netG
end
...
function defineG_gated_resnet_6blocks( input_nc, input_nclass, output_nc, ngf )
  padding_type = 'reflect'
  local ks = 3
  local netG = nil　　
  local f = 7
  ...
  local style_label = -nn.Identity() -- should be (batchsize, input_nclass)
  local e1 = content- nn.SpatialReflectionPadding(p, p, p, p) - nn.SpatialConvolution(input_nc, ngf, f, f, 1, 1) - normalization(ngf) - nn.ReLU(true) --(3)conv
  local e2 = e1 - nn.SpatialConvolution(ngf, ngf*2, ks, ks, 2, 2, 1, 1) - normalization(ngf*2) - nn.ReLU(true) --(6)conv
  local e3 = e2 - nn.SpatialConvolution(ngf*2, ngf*4, ks, ks, 2, 2, 1, 1) - normalization(ngf*4) - nn.ReLU(true) -- (9)conv

  local t = nn.ConcatTable()
  for i =1, input_nclass do
    local t_i = nn.Sequential()
     t_i:add( build_res_block(ngf*4, padding_type))
    t:add(t_i)
  end
  local gated_module = nn.ParallelTable():add(nn.Identity):add(t)
  local gated_t = {style_label, e3}-gated_module-nn.MixtureTable()

  local d1 = gated_t - build_res_block(ngf*4, padding_type) - build_res_block(ngf*4, padding_type) - build_res_block(ngf*4, padding_type) - build_res_block(ngf*4, padding_type) - build_res_block(ngf*4, padding_type)
  local d2 = d1 - nn.SpatialFullConvolution(ngf*4, ngf*2, ks, ks, 2, 2, 1, 1,1,1) - normalization(ngf*2) - nn.ReLU(true)
  local d3 = d2 - nn.SpatialFullConvolution(ngf*2, ngf*1, ks, ks, 2, 2, 1, 1,1,1) - normalization(ngf) - nn.ReLU(true)
  local d4 = d3 - nn.SpatialReflectionPadding(p, p, p, p) - nn.SpatialConvolution(ngf, output_nc, f, f, 1, 1) - nn.Tanh()
  netG = nn.gModule({content,style_label},{d4})
  return netG
end

function defineG_auto_gated_resnet_6blocks( input_nc, input_nclass, output_nc, ngf )
  padding_type = 'reflect'
  local ks = 3
  local netG = nil　　
  local f = 7
  ...
  t:add(nn.Identity)
  local gated_module = nn.ParallelTable():add(nn.Identity):add(t)
  local gated_t = {style_label, e3}-gated_module-nn.MixtureTable() --(13:gated_module)
...
function copy_weight(net_pre, net, nstyle)
  for i=1,12 do
    local params_pre, gradParams_pre = net_pre:get(i):getParameters()
    local params, gradParams = net:get(i):getParameters()
    params:copy(params_pre)
  end

  for i=1,nstyle do
    local params_pre, gradParams_pre = net_pre:get(13):get(2):get(i):getParameters()
    local params, gradParams = net:get(13):get(2):get(i):getParameters()
    params:copy(params_pre)
  end
  for i=15,28 do
    local params_pre, gradParams_pre = net_pre:get(i):getParameters()
...
function concat_class_layer (net_orig, use_sigmoid, n_layers, ndf, input_nclass,kw)
  --add one table
  -- reshape the second table
  --join
  local net_len=net_orig:size()
  net_orig:remove(net_len)
  net_len = net_len -1
  if use_sigmoid then
    net_orig:remove(net_len)
    net_len = net_len -1
  ...
  local f_mult = math.min(2^n_layers,8)
  local padw = math.ceil((kw-1)/2)
  local nf_mult = math.min(2^n_layers,8)
  local concat1 = nn.Sequential()
  concat1:add(nn.SpatialConvolution(ndf * nf_mult, 1, kw, kw, 1, 1, padw,padw))
  if use_sigmoid then
    concat1:add(nn.Sigmoid)
  end

  -- class layer part
  local class_layer=nn.Sequential()
  class_layer:add(nn.SpatialConvolution(ndf*f_mult, input_nclass, kw, kw, 1, 1, padw, padw))
  class_layer:add(nn.Transpose({2,4}))
  -- class_layer:add(nn.View(-1):setNumInputDims(3))

  concat:add(concat1)
  concat:add(class_layer)
  net_orig:add(concat)
