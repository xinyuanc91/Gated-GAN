...
--
function visualizer.save_results(visuals, opt, epoch, counter, phase)
  if phase == nil then
    phase ='train'
  end
  local tensortype = torch.getdefaulttensortype()
  ...
  out_path = paths.concat(opt.checkpoints_dir,  opt.name, 'epoch' .. epoch .. '_iter' .. counter .. '_' .. phase .. '_res.png')
  if #image_out:size()>3 then
...
function visualizer.save_images(imgs, save_dir, impaths, s1, s2)
  ...
  for i = 1, batchSize do -- imgs_f[i]:size(2), imgs_f[i]:size(3)/opt.aspect_ratio
    ...
    image.save('test.png',img_to_save)
